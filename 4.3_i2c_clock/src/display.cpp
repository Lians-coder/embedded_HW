...
void Display::sendCmds(uint8_t cmd)
{
  ...
  i2c_master_transmit(devOled, buf, sizeof(buf), I2C_TIMEOUT_MS);
}
...
void Display::sendData(uint8_t *data, size_t len)
{
  ...
  i2c_master_transmit(devOled, buf, len + 1, I2C_TIMEOUT_MS);
}
...
void Display::drawAll(tm dt, float t, TickType_t now)
{
  clear();
  ...
  drawString(FONT_H + 1, buf);

  // temp
  int tempInt = (t >= 0) ? (int)(t + 0.5f) : (int)(t - 0.5f);  // round

  if (tempInt > 0)
  {
    snprintf(buf, sizeof(buf), "+%ddC", tempInt);
  }
  else if (tempInt <= 0)
  {
    snprintf(buf, sizeof(buf), "%ddC", tempInt);
  }
  drawString((FONT_H + 1) * 2, buf);
}
